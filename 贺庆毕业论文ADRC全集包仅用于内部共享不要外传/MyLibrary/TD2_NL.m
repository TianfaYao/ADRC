function [sys,x0,str,ts] = TD2_NL(t,x,u,flag,r,h,h0)
...
switch flag,
  ...
  case 2 % 离散状态的更新
    sys = mdlUpdates( x,u,r,h,h0) ;
  case 3 % 计算输出信号
    sys = mdlOutputs(x) ;
  case 4
    sys=mdlGetTimeOfNextVarHit(t,x,u,h);
  case{1,9} % 不需要的值
    sys = [];
  otherwise % 错误处理
...
ts = [h 0]; % 采样周期
% ＊＊＊＊＊＊＊flag = 2， 更新离散系统状态变量 ＊＊＊＊＊＊
function sys = mdlUpdates( x,u,r,h,h0)
sys(1) = x(1) + h* x(2) ;
sys(2) = x(2) + h* fhan(x(1)-u,x(2),r,h0);
% ＊＊＊＊＊＊＊＊＊＊＊flag = 3， 计算输出量 ＊＊＊＊＊＊＊＊＊＊＊
